"""Omni Analyzer — Universal file analysis tool.

Analyzes ANY type of file via URL or attachment:
...
Works with:
1. Previously uploaded files in the conversation
2. Any public URL (http/https) to a file
3. Base64 encoded data

Uses the Omni model (configurable in settings) to analyze content.
The agent can analyze ANY file URL, not just context attachments.
"""
...
def omni_analyzer(file_source: str, query: str, config: RunnableConfig) -> str:
    """Universal file analysis tool — analyze any file via URL, attachment, or data.

    This tool can analyze ANY type of content:
    - Images: photos, screenshots, diagrams, charts
    - Audio: voice messages, podcasts, interviews
    - Video: screen recordings, tutorials, clips
    - Documents: PDFs, text files, code files

    Works with:
    - Previously uploaded files in conversation (pass filename)
    - Public URLs to any file (pass the full URL)
    - Any accessible file source

    Args:
        file_source: File name from conversation, or ANY public URL (http/https) to analyze (e.g., 'photo.jpg', 'https://example.com/video.mp4', 'document.pdf').
        query: What you want to know or do with the file (e.g., 'What objects are in this image?', 'Summarize the key points', 'What was said at timestamp 2:30?', 'Extract all text').

    Returns:
        Detailed analysis of the file content based on your query.
    """
    print(f"[omni_analyzer] Tool called for source: {file_source}, query: {query}")
    lower_source = file_source.lower()
    ...
    # Determine if file_source is a URL or a filename
    is_url = lower_source.startswith(('http://', 'https://'))
    file_url = None
    ...
    if is_url:
        ...
        if not filename:
            filename = 'attachment'
        print(f"[omni_analyzer] Direct URL provided: {file_url}")
    else:
        # Filename provided - search in conversation history
        ...
        # Fetch thread history from Supabase to find the file URL
        try:
            ...
            print(f"[omni_analyzer] DB lookup failed: {db_err}")
            return f"❌ Error: Database error looking up attachment '{file_source}'."

        if not file_url:
            return f"❌ Error: Could not find any uploaded file named '{file_source}' in this chat history. Please provide a direct URL instead."
    ...
    try:
        ...
        print(f"[omni_analyzer] Expiration head check failed: {head_err}")
    ...
    mime_type = ""
    try:
        print(f"[omni_analyzer] Downloading file content: {file_url}")
        ...
        if not mime_type:
            mime_type, _ = mimetypes.guess_type(lower_source)
        if not mime_type:
            mime_type = "application/octet-stream"
    except Exception as download_err:
        print(f"[omni_analyzer] Failed to download/encode file: {download_err}")
        return f"❌ Error: Failed to download the file '{filename}' from storage. Detail: {download_err}"

    # Text instruction & query
    instruction = (
        f"You are analyzing the file: {filename}\n"
    ...
    if omni_provider == "gemini":
        ...
        try:
            ...
            print(f"[omni_analyzer] Running Direct Gemini Omni analysis on {filename} with model {omni_model}...")
            ...
            print(f"[omni_analyzer] Direct Gemini analysis completed successfully.")
        ...
        try:
            print(f"[omni_analyzer] Running Omni fallback on {filename} via {omni_provider} with model {omni_model}...")
            resp = requests.post(
