...
# NOTE: _MODEL and _GATEWAY_BASE are now dynamic — resolved at call time via get_llm_config("analyzer")
_ANALYZER_DEFAULT_MODEL = "moonshotai/kimi-k2.5"
...
def analyze_images_gemini(image_urls: list[str]) -> str:
    ...
    # ── Call Vision LLM (provider/model from Supabase agent_settings) ────────────────────
    gateway_base, api_key, active_model = get_llm_config("analyzer")
    if not api_key:
        return "\u274c No API key configured for the analyzer. Check agent_settings or environment."
    ...
    try:
        print(f"[analyze_images_gemini] Sending {len(valid_urls)} candidates + 2 ref images to {active_model} via {gateway_base[:40]}...")
        resp = requests.post(
        ...
        return f"\u274c Vision API error ({active_model}): {e}"
