...
description: >
  Content Creation Skill — guidelines and frameworks for creating effective
  blog posts and social media content. Covers structure templates, writing
  best practices by channel, SEO fundamentals, headline & hook formulas,
...
## 2 — Writing Best Practices by Channel
...
**Twitter/X:**
- Concise and punchy — strong opening words
- Under 280 characters — every word earns its place
- Threads for longer narratives
- Lead with the most surprising fact or sharpest take
- End with a question or observation that invites engagement
- No hashtags required — one max if highly relevant

**Instagram:**
- Visual-first captions — the image hooks, the caption rewards
- Storytelling hooks in the first line (before "more" is cut off)
- Line breaks for readability — short paragraphs, white space
- 5-8 hashtags at the end (mix broad + niche)
- Emoji used sparingly for tone, not decoration

**Facebook:**
- Conversational tone — write like you are talking to a neighbour, not presenting a report
- Questions drive comments — end posts with a genuine question
- Lead with the most human angle of the story
- Include the source link with a brief editorial tease above it
- Keep the opening line under 80 characters to avoid truncation
...
## 3 — SEO Fundamentals for Content
...
## 6 — Blog Post Output Rules

When writing the blog post, save it to `/blog_post.md` using `write_file()`.

The file must start with a `---` frontmatter block:

```
---
title: [SEO title — 50-60 characters, includes focus keyword]
slug: [kebab-case-url-slug, max 6 words]
meta_description: [150-160 chars, includes focus keyword, compelling CTA]
focus_keyword: [primary keyword phrase, 2-4 words]
category_hint: [pakistan | sports | business | latest]
date: [TODAY'S DATE]
author: The Echo Desk
reading_time: [X min read]
---
```

**No citation numbers** (`[1]`, `[2]`, `[3]`) anywhere in the post body. Attribute facts naturally inside the sentence:
- ✅ `"The Petroleum Division confirmed petrol would remain at Rs. 321.17 per litre."`
- ✅ `"According to the finance minister's statement in the National Assembly..."`
- ❌ `"Petrol prices have been frozen for two consecutive fortnights [1][2]."`

**Image placeholders** — include both in the body exactly as written:
```
<!-- BLOG_IMAGE_1 -->
```
```
...
**(CRITICAL IMAGE RULE):** If you ever need to manually embed an image into the blog post yourself because the `embed_images_in_blog` tool failed, you **MUST ONLY use the original hosted `http` or `https` URL**. 
❌ **NEVER** use local file paths (like `output/candidate_images/image_1.jpg`) in the blog post body. Local paths break WordPress images.

### WordPress Image Sizing Rules (URL Parameters)
When modifying or manually embedding image URLs, append the correct parameters for the context:
- **Featured Image (1200x628):** `...jpg?w=1200&h=628&crop=1`
- **In-Post Large (800x500):** `...jpg?w=800&h=500&crop=1`
- **In-Post Medium (600x400):** `...jpg?w=600&h=400&crop=1`
- **Thumbnail:** `...jpg?w=300&h=200&crop=1`
- **Sweet Spot for Content Area:** Just append `?w=800` (e.g. `...jpg?w=800`) to resize width automatically keeping original proportions. Remove `&crop=1` if you want the image to scale without any cropping.

No Sources section. No FAQ section. No bibliography.

---

## 7 — Social Media Post Output Rules

When writing social media posts, save to `/social_posts.md` using `write_file()`.
...
## X (Twitter)
[Tweet — max 280 characters. Strong opening word. No hashtags or one max.]

## Instagram
[Caption — hook + story + hashtags. Line breaks for readability.]
#hashtag1 #hashtag2 #hashtag3 #hashtag4 #hashtag5

## Facebook
[Post — conversational, question at end, full context for readers who haven't seen the headline.]
